...
def initialize_population(size: int)-> List[str]:
    ...
    return  [''.join(random.choices('01', k=23)) for _ in range(size)]
...
def genetic_algorithm(population:List[str], fitness:List[float], cross_rate: float, mutation_rate: float) -> List[str]:
